...
def draw_player_stats(output_video_frames,player_stats):
    ...
    for index, row in player_stats.iterrows():
        ...
        # box ki position frame ke bottom-right corner se fix hai (hardcoded offset)
        start_x = frame.shape[1]-400
        start_y = frame.shape[0]-500
        end_x = start_x+width
        end_y = start_y+height

        # semi-transparent black box banane ka trick: overlay draw karo, phir addWeighted se blend karo
        overlay = frame.copy()
